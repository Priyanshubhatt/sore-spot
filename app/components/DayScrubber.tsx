...
export default function DayScrubber({ asOf, day, onChange }: Props) {
  ...
  return (
    <View
      ...
      accessibilityValue={{ text: dateLabel(asOf, day) }}
      {...pan.panHandlers}
    >
      {Array.from({ length: FORECAST_DAYS }, (_, d) => (
        ...
        <View key={d} pointerEvents="none" style={[styles.cell, d === day && styles.cellSelected]}>
          <Text
            style={[styles.label, d === day && styles.labelSelected]}
            numberOfLines={1}
...
const styles = StyleSheet.create({
  ...
  cell: { flex: 1, alignItems: 'center', paddingVertical: 9, paddingHorizontal: 1 },
  cellSelected: { backgroundColor: colors.accent },
  label: { fontSize: 11.5, fontWeight: '700', color: colors.text },
