...
8. Refuses to write an export with **no workouts** (the app would show a blank map) and says to try a longer `--days`. Otherwise writes `data/replay.json` (owner-only permissions where the system supports them) and prints counts only: workouts and date range, recovery records, sports found, **which sports the model has no muscle map for** (they add no soreness), and how many strength sessions will need a tag (only those in the last 8 days: the soreness curve reaches zero after 192 hours, so older sessions change nothing and are never asked about). Nothing personal (no ids) is printed, so the summary can be pasted back for review.
...
- Strength sessions arrive untagged, so the existing tag prompt and the plan's "some workouts are not counted" note work unchanged, but an untagged or unmapped session older than the curve's 192 hours is no longer reported. Session labels use the workout's own `timezone_offset`, so the day shown is the day it happened; unmapped sports show the existing "not mapped" note.
